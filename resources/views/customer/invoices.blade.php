<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('My Invoices') }}</h2></x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-6">Invoice History</h3>
                    ...
                    <div class="space-y-4">
                            ...
                            <div class="flex justify-between items-start">
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-2">
                                        <h4 class="text-lg font-semibold text-gray-900">Invoice #{{ $invoice->id }}</h4>
                                    ...
                                    <div class="grid grid-cols-2 gap-4 text-sm text-gray-600 mb-3">
                                        <div>
                                            <p class="font-medium text-gray-700">Pet</p>
                                        ...
                                        <div>
                                            <p class="font-medium text-gray-700">Amount</p>
                                            <p class="text-pink-600 font-semibold text-lg">Rp {{ number_format($invoice->amount, 0, ',', '.') }}</p>
                                        </div>
                                    </div>
                                    @endif

                                    <p class="text-xs text-gray-400">Created: {{ $invoice->created_at->format('d M Y, H:i') }}</p>
                                    @if($invoice->paid && $invoice->updated_at)
                                        <p class="text-xs text-gray-400">Paid: {{ $invoice->updated_at->format('d M Y, H:i') }}</p>
                                    @endif
                                </div>
                                <div class="ml-4">
                        ...
                        <div class="text-center py-12">
                            ...
                            <p class="mt-4 text-gray-500">No invoices yet.</p>
                        </div>
                        @endforelse
                    </div>
