<x-app-layout>
        ...
        <div class="bg-white rounded-xl shadow-sm mt-6">
            <form action="{{ route('customer.storeBooking') }}" method="POST">
                ...
                <div class="p-6">
                    <!-- Room Info -->
                    <div class="mb-6 p-4 bg-gradient-to-r from-green-50 to-teal-50 rounded-lg border border-green-100">
                        ...
                        <p class="text-2xl font-bold text-primary mt-2">Rp {{ number_format($room->rate_per_day, 0, ',', '.') }}{{ __('messages.per_day') }}</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="start_date" id="start_date" value="{{ old('start_date') }}" required min="{{ date('Y-m-d') }}" class="form-input w-full @error('start_date') border-red-500 @enderror">
                            @error('start_date')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label for="end_date" class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.check_out_date') }} *</label>
                            <input type="date" name="end_date" id="end_date" value="{{ old('end_date') }}" required min="{{ date('Y-m-d') }}" class="form-input w-full @error('end_date') border-red-500 @enderror">
                ...
                <div class="p-6 bg-gray-50 rounded-b-xl border-t flex justify-end gap-4">
                    ...
                    <button type="submit" class="btn-gradient inline-flex items-center px-4 py-2 rounded-md font-semibold text-xs uppercase tracking-widest transition">{{ __('messages.confirm_booking') }}</button>
                </div>
            </form>
        </div>
    </div>
