            ...
            <form action="{{ route('bookings.update', $booking) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="pet_id" id="pet_id" required class="form-input w-full">
                            @foreach($pets as $pet)
                    ...
                    <div>
                        ...
                        <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $booking->start_date) }}" required class="form-input w-full">
                    </div>

                    <!-- End Date -->
                    <div>
                        <label for="end_date" class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.end_date') }} *</label>
                        <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $booking->end_date) }}" required class="form-input w-full">
                    </div>
                ...
                <div class="mt-8 flex items-center justify-end gap-4">
                    ...
                    <button type="submit" class="btn-gradient inline-flex items-center px-4 py-2 rounded-md font-semibold text-xs uppercase tracking-widest transition">{{ __('messages.update_booking') }}</button>
                </div>
            </form>
        </div>
    </div>
